...
- Prefer **theme tokens and named utilities** over arbitrary values.
- Design tokens live in `registry/styles/packed-theme-tokens.css` and
  `app/globals.css`. Add missing tokens there first, then use generated classes
  (`bg-app`, `text-muted-foreground`, `border-border-subtle`, `rounded-lg`,
  `shadow-card`, …).
- Reuse role elevation aliases (`--elev-card` / `--elev-popover` / `--elev-modal`)
  — do not restyle each component.
